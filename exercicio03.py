...
def criar_produto(nome: str, preco: float, estoque: int = 0):
    item = {"nome": nome, "preco": preco, "estoque": estoque}
    product_list.append(item)
    return item


def aplicar_desconto(produto: dict, percentual: float):
    novo_preco = produto["preco"] - produto["preco"] * percentual / 100

    novo_produto = {**produto, "preco": novo_preco}

    return novo_produto


def resumo(*produtos: dict):
