...
| 21 | ⚡ Background Tasks: `BackgroundTasks` injetado como parâmetro, `add_task(fn, args)` = "rode depois de responder" (resposta na hora, ~2ms vs ~2s), aplicado no email de boas-vindas do `POST /usuarios`. Fire-and-forget: se a task falha o usuário não sabe (já recebeu sucesso), erro só no log, sem retry → leve/não-crítico = BackgroundTasks; pesado/crítico = fila (Celery/RQ+Redis) | ✅ |
| 22 | ✅ Validação de entrada (Pydantic ≈ Zod): tipo já valida (→ 422 automático na fronteira), `EmailStr` valida formato (não muda coluna, sem migration), `Field(min_length/gt/le/pattern)` = restrições declarativas, `@field_validator` (método DA classe — indentação importa!) p/ regra customizada (`ValueError` → 422). Apertar validação quebra testes que usavam dado frouxo (lição) | ✅ |
